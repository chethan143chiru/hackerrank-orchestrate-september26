...
class CalibratedSimulator:
    ...
    def parse_user_context(self, user_id, req_date):
        ...
        for e in user_events:
            if 'Final employer payroll' in e['description'] or 'Previous employer payroll' in e['description']:
                salary_info['confirmed'] = False
            if 'Next confirmed salary' in e['description'] and e['status'] in ('scheduled', 'confirmed'):
                salary_info['confirmed'] = True
                s_date = parse_date(e['event_date'])
                salary_info['pay_day'] = s_date.day
                salary_info['first_date'] = s_date
                amt = float(e['amount']) if e['amount'] else 0.0
                salary_info['amount'] = self.convert_currency(amt, e['currency'], home_curr, e['event_date'])
        ...
        for m in user_msgs:
            ...
            if m_date:
                ...
                salary_info['pay_day'] = salary_info['first_date'].day
    ...
    def forecast_cash_flow(self, user_id, req_date, days=90, spending_changes=None):
        ...
        for cat, evs in cat_streams.items():
            ...
            mode = Counter(diffs).most_common(1)[0][0] if diffs else 30
            ...
            if mode >= 25 or cat in ('rent', 'housing', 'utilities', 'insurance', 'debt_repayment', 'education', 'healthcare'):
                ...
                interval = mode if mode > 0 else 7
    ...
    def evaluate_request(self, req):
        ...
        for d_offset in range(len(base_curve)):
            d = req_date + timedelta(days=d_offset)
            # Full payment safe on date d if balance(d) - req_amt >= min_bal
            # and min surplus in next 30 days remains >= min_bal
            safe = True
            end_window = min(len(base_curve), d_offset + 30)
            for t in range(d_offset, end_window):
                if (base_curve[t] - req_amt) < min_bal - 10.0:
                    safe = False
                    break
            if safe:
        ...
        for opt in options:
            if opt['payment_method'] == 'installments' and 'installments' in pref_methods:
                ...
                # Check if fits balance curve during the plan period
                safe = True
                ...
                cur_inst_paid = 0.0
                last_off = (last_pay_date - req_date).days
                check_window = min(len(base_curve), max(last_off + 1, 30))
                for t in range(check_window):
                    cur_inst_paid += cum_inst[t]
                    if (base_curve[t] - cur_inst_paid) < min_bal - 10.0:
                        safe = False
                        break
        ...
        # 4. Full payment with spending changes
        if amount_safe_to_pay < req_amt - 1e-4 and 'full_payment' in pref_methods:
            ...
            for eid, savings, desc in eligible_stops:
                ...
                if min(c[t] - min_bal for t in range(len(c))) >= req_amt - 100.0:
                    found_changes = sc
                    break
...
sim = CalibratedSimulator('dataset')
...
for s in samples:
    ...
    print(f"[{s['request_id']}] Status {status_icon}: {res['affordability_status']:<20} (Exp: {s['affordability_status']}) | Method {method_icon}: {res['recommended_payment_method']:<16} (Exp: {s['recommended_payment_method']})")
    if not (match_status and match_method):
        print(f"  Plan: {res['payment_plan']}")
        print(f"  Exp : {s['payment_plan']}")

print(f"\nCalibrated Accuracy Summary:")
